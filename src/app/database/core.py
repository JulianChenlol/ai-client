...
# from src.search.fulltext import make_searchable
    ...
    if not mapped_class:
        raise ValidationError(
            [
                InitErrorDetails(
                    type=NotFoundError(
                        message="Model not found. Check the name of your model.", code=None
                    ),
                    input=mapped_name,
                )
            ],
...
def refetch_db_session(organization_slug: str) -> Session:
    schema_engine = engine.execution_options(
        schema_translate_map={
            None: f"app_organization_{organization_slug}",
        }
    )
